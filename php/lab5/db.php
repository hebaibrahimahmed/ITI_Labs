...
public function insertInto($connection,$table,$useremail,$username,$userpassword,$userroom,$image_new_name){
    try{
    $query="Insert INTO `$table` (`name`, `email`,`password`,`Room`,`image`) Values(:username,:useremail,:userpassword,:userroom,:userimage)";
...
public function updatefromTable( $connection,$table,$id,$useremail,$username,$userpassword,$userroom,$image_new_name){

    try{
     $query = "update $table set `Name`=:username, `Email`=:useremail, `password`=:userpassword, `room`=:userroom, `image`=:userimage   where id=:id ";
     $stmt=$connection->prepare($query);
  //    var_dump($query);
  //    var_dump($stmt);
     $stmt->bindParam(":useremail", $useremail, PDO::PARAM_STR);
